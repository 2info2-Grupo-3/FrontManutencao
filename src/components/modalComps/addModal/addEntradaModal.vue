...
<template>
    ...
    <div class="modalAdd">
      <div class="modalHeader">
        ...
        <form @submit.prevent>
            <div><input type="date" placeholder="Data" v-model="entrada.data" /></div>
            <div><input type="number" placeholder="Quantidade" v-model="entrada.quantidade" /></div>
               <div> <select name="" id="" v-model="entrada.peca">
                    <option v-for="peca in pecas" :key="peca.id" :value="peca.id">{{ peca.nome }}</option>
                </select>
            </div>
        </form>
        <div class="container-btn-add">
        <button class="btn-add" @click="entradasStore.createEntrada(entrada)">Adicionar</button>
      </div>
    </div>
    </div>
</template>
