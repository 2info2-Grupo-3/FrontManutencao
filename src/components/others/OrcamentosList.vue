...
function filteredList() {
  return orcamentos.value.filter((itemf) =>
    itemf.nome_cliente.toLowerCase().includes(inputSearch.value.toLowerCase())
  )
}
        ...
        <div class="modalInfo">
          ...
          <div class="itemInfo">
            ...
            <select name="" id="" v-model="servicoAdd">
              <option :value="servico" v-for="servico in servicos" :key="servico.id">
                {{ servico.nome }}
              </option>
            </select>
          ...
          <div class="itemInfo-btn"></div>
        </div>
        <div class="itemInfo-btn">
          <button @click="orcamentosStore.updateOrcamento(selectedOrcamento.id, selectedOrcamento)">
            Atualizar
          </button>
